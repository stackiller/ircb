...
int new_conn(const char *hostname, const char *port) {
  int sockfd, s;
  ...
  for (next = res; next != NULL; next = next->ai_next) {
    ...
    if (sockfd > 0) {
      memcpy(&addr_copy, next->ai_addr, next->ai_addrlen);
      addrlen_copy = next->ai_addrlen;
      break;
    }
  }
  ...
  if ((connect(sockfd, (struct sockaddr *)&addr_copy, addrlen_copy)) != 0) {
  ...
  if (SSL_connect(irc.ssl) == 0) {
    ...
    printf("[%s%s*%s] Connected with %s.\n", fGreen, fBlink, fRs,
           SSL_get_cipher(irc.ssl));
  }
...
void show_Certs(SSL *ssl) {
  ...
  if (cert != NULL) {
    ...
    // get issuer name
    line =
        X509_NAME_oneline(X509_get_issuer_name(cert), 0, 0); // get the issuer.
